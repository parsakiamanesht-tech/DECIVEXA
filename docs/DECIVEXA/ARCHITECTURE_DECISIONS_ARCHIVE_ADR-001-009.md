# DECIVEXA — Architecture Decisions Archive (ADR-001–ADR-009)

**Purpose:** this document is the live, repository-resident archive of
the full, detailed decision text of ADR-001 through ADR-009. It exists
to restore direct repository readability for these decisions after
`docs/DECIVEXA/ARCHITECTURE_DECISIONS.md` was restructured (commit
`9582d8575e059d44dad1579bb65e0d227a5a205e`, 2026-09-06) to register
ADR-010, at which point ADR-001–009's detailed text was replaced there
by a summary placeholder.

**Historical source commit:** `dfdd798ce1380793313f0726245349deb593a2d5`
("docs(architecture): register ADR-009", 2026-08-25) — the last commit
to `docs/DECIVEXA/ARCHITECTURE_DECISIONS.md` before the restructuring,
confirmed to contain the complete, unmodified text of ADR-001 through
ADR-009. The text below is copied verbatim from that commit, with no
paraphrasing, summarization, modernization, or correction.

**Governance status of this document:**

1. ADR-001 through ADR-009 are historical Founder-approved architectural
   decisions.
2. Their detailed text is preserved here, byte-for-byte, from the
   historical source commit identified above.
3. This document exists solely to maintain live repository readability
   and governance traceability for these decisions — it is a
   relocation, not a new decision.
4. This relocation does not supersede, reinterpret, or modify the
   original decisions in any way. Their architectural authority and
   scope remain exactly as originally recorded.
5. A later ADR may supersede an individual decision recorded here only
   where that later, governing record explicitly and specifically says
   so. Absent such an explicit statement, every decision below remains
   authoritative within its originally recorded scope.

**Current authoritative index:** see
`docs/DECIVEXA/ARCHITECTURE_DECISIONS.md` for ADR-010 onward, and for
the cross-reference back to this archive.
